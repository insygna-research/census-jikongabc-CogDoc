...
**CogDoc** 是一个面向**学术论文与技术文档**的开源 RAG 知识库系统，上层由 **LangGraph 多 Agent 编排**驱动，底层由**确定性 Rust 核心（PyO3 + maturin）**加速。与那些对所有文档一视同仁的通用 RAG 聊天机器人不同，CogDoc 理解科学文献的结构——PDF 按章节、图表、参考文献智能解析，每条生成结论都锚定到 `[source:Pn]` 引用，并且这些引用是**经过校验的，而非默认可信**。

面向个人研究者、研发团队以及任何被 PDF 淹没的人，CogDoc 让你能够**基于自己的私有知识库做有据可依的问答**、**生成单篇文档的结构化摘要**、**多篇论文按标准化维度横向对比**——全都跑在你自己硬件上。数据不离开你的机器，不需要向量搜索的 API key，没有意外。

CogDoc 的与众不同之处：

- **引用自愈** — Rust 校验循环会捕获捏造的文件名和页码，把回答打回重新生成，让你永远不会信任一条幻觉引用。
- **中英文都好用的混合检索** — 向量（Chroma + 多语言 BGE-M3）和 BM25 两路召回由 Rust RRF kernel 融合。中文通过 `jieba-rs` 分词，英文做 Snowball 词干化 + 停用词过滤，全部 native 执行。
- **内容寻址的增量索引** — SHA-256 manifest 和带版本的 chunk 契约确保未变化的文件直接复用缓存索引。10 个文档的知识库重入库不到一秒，如果什么都没变的话。
- **多知识库 · 多对话 · 持久记忆** — 每个知识库独立运行多个对话，历史通过 SQLite 长期持久化，短期滑动窗口（默认最近 12 轮）支持多轮指代消解。只有通过引用校验的回答才写入记忆，错误生成不会毒害后续轮次。
- **双前端，同一引擎** — 面向高级用户的斜杠命令 CLI 控制台，以及基于 FastAPI、支持流式回答、来源检查和 👍/👎 反馈的 Streamlit 网页端。
