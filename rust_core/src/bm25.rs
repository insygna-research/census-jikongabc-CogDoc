...
impl Bm25Index {
    ...
    fn new(tokenized_corpus: Vec<Vec<String>>, k1: f64, b: f64, epsilon: f64) -> Self {
        ...
        for (doc_id, doc) in tokenized_corpus.iter().enumerate() {
        ...
        for (word, freq) in &nd {
            let value =
                (corpus_size as f64 - *freq as f64 + 0.5).ln() - (*freq as f64 + 0.5).ln();
        ...
        for (token, count) in &query_counts {
            ...
            if let Some(plist) = self.postings.get(*token) {
                for (doc_id, freq) in plist {
                    let denom =
                        *freq + self.k1 * (1.0 - self.b + self.b * self.doc_len[*doc_id] / self.avgdl);
