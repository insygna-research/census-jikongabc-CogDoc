...
def current_index_provenance(kb_id: str) -> dict[str, Any]:
    ...
    Generation state is the transactional authority.  The manifest supplements it
    with the chunk-identity contract and remains a compatibility fallback for
    legacy indexes.  Observability must never make chat unavailable, so corrupt or
    absent provenance degrades to explicit empty fields.
    """
    ...
    return {
        ...
        "chunk_identity_version": str(manifest.get("chunk_identity_version") or ""),
