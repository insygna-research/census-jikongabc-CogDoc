...
# 验证派生知识引用可以通过校验。
def test_valid_knowledge_citation_passes():
    docs = [
        {
            "text": "补充规则。",
            "meta": {
    ...
    result = validate("差旅报销需要七天内提交。[knowledge:K123]", docs=docs)
...
# 验证非法派生知识引用会被拒绝。
def test_invalid_knowledge_citation_rejected():
    docs = [
        {
            "text": "补充规则。",
            "meta": {
    ...
    result = validate("差旅报销需要七天内提交。[knowledge:K999]", docs=docs)

    assert result["is_valid"] is False
    assert "派生知识引用不存在" in result["critique"]
