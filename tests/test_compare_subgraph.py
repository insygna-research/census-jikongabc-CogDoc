...
def test_document_loader_limits_local_compare_sources(monkeypatch):
    ...
    result = document_loader_node({"query": "请对比 a.pdf、b.pdf 和 c.pdf", "doc_id": "kb", "is_local": True})
...
def test_document_loader_uses_limited_local_dimensions(monkeypatch):
    ...
    result = document_loader_node({"query": "请对比 a.pdf 和 b.pdf", "doc_id": "kb", "is_local": True})

    assert [dimension["title"] for dimension in result["compare_dimensions"]] == ["方法", "数据", "指标", "限制"]


def test_document_profile_node_generates_cells_with_evidence(monkeypatch):
    monkeypatch.setattr(compare_profile.Generator, "_get_client", lambda is_local = False: FakeCompareLLM())
    ...
    assert [profile["source"] for profile in result["document_profiles"]] == ["a.pdf", "b.pdf"]
    assert [cell["dimension_id"] for cell in result["document_profiles"][0]["cells"]] == ["method", "metrics"]
    assert result["document_profiles"][0]["cells"][0]["content"] == "该维度内容来自文档。[a.pdf:P1]"
    assert result["document_profiles"][1]["cells"][0]["content"] == "该维度内容来自文档。[b.pdf:P2]"
    assert result["document_profiles"][0]["cells"][0]["evidence"][0]["chunk_id"] == "chunk:a.pdf:0"


def test_document_profile_node_keeps_no_evidence_cells_empty(monkeypatch):
    monkeypatch.setattr(compare_profile.Generator, "_get_client", lambda is_local = False: FakeNoEvidenceLLM())
...
def test_compare_table_node_builds_dimension_blocks_and_conclusion(monkeypatch):
    monkeypatch.setattr(
        ...
        lambda state, table_answer: "a.pdf 强调方法，b.pdf 强调指标。[a.pdf:P1][b.pdf:P2]",
    )
    state = {
        "compare_sources": ["a.pdf", "b.pdf"],
        "compare_docs_by_source": {
        ...
        "document_profiles": [
            {
                "source": "a.pdf",
                "cells": [
                    {"dimension_id": "method", "source": "a.pdf", "content": "方法 A。[a.pdf:P1]", "evidence": []},
                    {"dimension_id": "metrics", "source": "a.pdf", "content": "指标 A。[a.pdf:P1]", "evidence": []},
                ],
            },
            {
                "source": "b.pdf",
                "cells": [
                    {"dimension_id": "method", "source": "b.pdf", "content": "方法 B。[b.pdf:P2]", "evidence": []},
                    {"dimension_id": "metrics", "source": "b.pdf", "content": "指标 B。[b.pdf:P2]", "evidence": []},
                ],
            },
        ],
    }
...
def test_compare_table_node_records_conclusion_failure(monkeypatch):
    ...
    monkeypatch.setattr(compare_generator.CompareGeneratorAgent, "_generate_conclusion", raise_timeout)
    state = {
        "compare_sources": ["a.pdf", "b.pdf"],
        "compare_docs_by_source": {
            "a.pdf": [_doc("a.pdf", 1, 0)],
...
def test_compare_table_node_skips_conclusion_in_local_mode(monkeypatch):
    ...
    monkeypatch.setattr(compare_generator.CompareGeneratorAgent, "_generate_conclusion", fail_if_called)
    state = {
        "compare_sources": ["a.pdf", "b.pdf"],
        "compare_docs_by_source": {
            "a.pdf": [_doc("a.pdf", 1, 0)],
...
def test_citation_node_downgrades_invalid_compare_citation(monkeypatch):
    monkeypatch.setattr(compare_generator.CompareGeneratorAgent, "_generate_conclusion", lambda state, table_answer: "")
    state = {
        "compare_sources": ["a.pdf", "b.pdf"],
        "compare_docs_by_source": {
            "a.pdf": [_doc("a.pdf", 1, 0)],
        ...
        "document_profiles": [
            {
                "source": "a.pdf",
                "cells": [
                    {"dimension_id": "method", "source": "a.pdf", "content": "方法 A。[a.pdf:P99]", "evidence": []},
                ],
            },
            {
                "source": "b.pdf",
        ...
        "document_profiles": [
            {
                "source": "a.pdf",
                "cells": [
                    {"dimension_id": "method", "source": "a.pdf", "content": "采用了方法 A。", "evidence": []},
                ],
            },
            {
                "source": "b.pdf",
                "cells": [
                    {"dimension_id": "method", "source": "b.pdf", "content": "采用了方法 B。", "evidence": []},
                ],
            },
        ],
    }
        ...
        "document_profiles": [
            {
                "source": "a.pdf",
                "cells": [
                    {"dimension_id": "method", "source": "a.pdf", "content": "文档中未明确说明。", "evidence": []},
                ],
            },
            {
                "source": "b.pdf",
                "cells": [
                    {"dimension_id": "method", "source": "b.pdf", "content": "文档中未明确说明。", "evidence": []},
                ],
            },
        ],
    }
...
def test_workflow_routes_to_compare_subgraph_smoke(monkeypatch):
    ...
    assert result["document_profiles"][0]["cells"][0]["evidence"][0]["source"] == "a.pdf"
    assert result["critique"] == ""
