...
def test_invoke_structured_caches_successful_auto_method(monkeypatch):
    ...
    assert (
        invoke_structured(llm, DemoSchema, [{"role": "user", "content": "json"}]).value
        == "plain"
    )
    assert llm.methods_seen == ["json_mode", "json_schema", "function_calling"]
...
def test_invoke_structured_cache_survives_client_recreation(monkeypatch):
    ...
    assert (
        invoke_structured(
            first, DemoSchema, [{"role": "user", "content": "json"}]
        ).value
        == "json_schema"
    )
    ...
    assert (
        invoke_structured(
            second, DemoSchema, [{"role": "user", "content": "json"}]
        ).value
        == "json_schema"
    )
